          ...
          <div className="w-full rounded-md p-3">
            {enrolledStudents.map((course, index) => (
              <div key={index} className="mb-4 bg-[#245De1] rounded-md p-1">
                <h3 className="text-[#245DE1] font-bold border-b border-[#245DE1] text-lg bg-white p-2">{course.courseName}</h3>
                <ul className="flex-flex-col gap-1 bg-white">
                  {course.students.map((student, studentIndex) => (
                    <li className='p-2 border-b' key={studentIndex}>{student}</li>
                  ))}
                </ul>
              </div>
            ))}
